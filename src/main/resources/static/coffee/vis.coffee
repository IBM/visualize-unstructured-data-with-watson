...
#bubble cloud source from Jim Vallandingham
...
Bubbles = () ->
  ...
  updateLabels = () ->
    ...
    #  styling divs
    label
      .style("font-size", (d) -> Math.max(8, rScale(rValue(d) / 6)) + "px")
      .style("width", (d) -> 1.5 * rScale(rValue(d)) + "px")
...
drawBubbles = (tabType) ->
  ...
  display = (data) ->
    ...
    if tabType is "#keywordsTab"
      plotData("#vis", dataObj.keywords, plot)
    if tabType is "#conceptsTab"
      plotData("#vis", dataObj.concepts, plot)
    if tabType is "#entitiesTab"
      plotData("#vis", dataObj.entities, plot)
    if tabType is "#sentimentsTab"
      plotData("#vis", dataObj.sentiment.document, plot)
...
# this is dirty, move it to json
drawSentiments = () ->
  svgContainer = d3.select("#vis").append("svg").attr("width",200).attr("height",200)
  rectangle1 = svgContainer.append("rect").attr("x", 150).attr("y", 60).attr("width", $("#joyScore").val()*2).attr("height", 20).style("fill", "#00C851")
  ...
  rectangle2 = svgContainer.append("rect").attr("x", 150).attr("y", 100).attr("width", $("#angerScore").val()*2).attr("height", 20).style("fill", "#ff4444")
  ...
  rectangle3 = svgContainer.append("rect").attr("x", 150).attr("y", 140).attr("width", $("#sadScore").val()*2).attr("height", 20).style("fill", "#4B515D")
  ...
  rectangle4 = svgContainer.append("rect").attr("x", 150).attr("y", 180).attr("width", $("#disgustScore").val()*2).attr("height", 20).style("fill", "#33b5e5")
  ...
  rectangle5 = svgContainer.append("rect").attr("x", 150).attr("y", 220).attr("width", $("#fearScore").val()*2).attr("height", 20).style("fill", "#ffbb33")
  ...
  if($("#positiveDocSentiment").val()>0)
   rectangle6 = svgContainer.append("rect").attr("x", 150).attr("y", 260).attr("width", $("#positiveDocSentiment").val()*2).attr("height", 20).style("fill", "#DCE9BE")
  ...
  if($("#negativeDocSentiment").val()>0)
   rectangle7 = svgContainer.append("rect").attr("x", 150).attr("y", 260).attr("width", $("#negativeDocSentiment").val()*2).attr("height", 20).style("fill", "#f06292")
...
$ ->
  ...
  $("#keywordsTab").click ->
    ...
    drawBubbles("#keywordsTab")
  $("#entitiesTab").click ->
    $("#conceptsLi").removeClass('active');
    $("#sentimentsLi").removeClass('active');
    $("#keywordsLi").removeClass('active');
    $("#entitiesLi").addClass('active');
    drawBubbles("#entitiesTab")
  $("#sentimentsTab").click ->
    $("#entitiesLi").removeClass('active');
    $("#sentimentsLi").removeClass('active');
    $("#keywordsLi").removeClass('active');
